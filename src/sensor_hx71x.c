...
// hx71x ADC query
void hx71x_query(struct hx71x_adc *hx71x, uint8_t oid)
{
...
void command_attach_endstop_hx71x(uint32_t *args)
{
    uint8_t oid = args[0];
...
void command_query_hx71x_status(const uint32_t *args)
{
    ...
    sensor_bulk_status(&hx71x->sb, oid, t1, (t2 - t1), pending_bytes);
}
