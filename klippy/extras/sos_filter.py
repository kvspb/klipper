...
class SosFilter:
    ...
    def __init__(self, mcu, fixed_point_filter, max_sections=None):
        self._mcu = mcu
    ...
    def _cmd(self, command, args, is_init=False, cq=None):
        if is_init:
            self._mcu.add_config_cmd(command[0] % args, is_init=True)
        else:
            cmd = self._mcu.lookup_command(command[1], cq=cq)
            cmd.send(args)

    def _set_filter_sections(self, is_init=False, cq=None):
        for i, section in enumerate(self._filter.get_filter_sections()):
            args = (self._oid, i, section[0], section[1], section[2],
                    section[3], section[4])
            self._cmd(self.cmd_set_section, args, is_init, cq)

    def _set_filter_state(self, is_init=False, cq=None):
        for i, state in enumerate(self._filter.get_initial_state()):
            args = (self._oid, i, state[0], state[1])
            self._cmd(self.cmd_config_state, args, is_init, cq)

    def _activate_filter(self, is_init=False, cq=None):
        args = (self._oid, self._filter.get_num_sections(),
                self._filter.get_coeff_int_bits())
        self._cmd(self.cmd_activate, args, is_init, cq)
    ...
    def _configure_filter(self, is_init=False, cq=None):
        ...
        self._set_filter_sections(is_init, cq)
        self._set_filter_state(is_init, cq)
        self._activate_filter(is_init, cq)
    ...
    def change_filter(self, fixed_point_filter, cq=None):
        self._filter = fixed_point_filter
        self._configure_filter(False, cq)

    # Resets the filter state back to initial conditions at runtime
    # cq is an optional command queue to for command sequencing
    def reset_filter(self, cq=None):
        self._set_filter_state(False, cq)
        self._activate_filter(False, cq)
