...
class ParamHelper:
    def __init__(self, config, name, type_name, default=None, minval=None,
        ...
        # read from config, default becomes config value, if present
    ...
    def _validate_float_list(self, gcmd, values, above, below):
        if gcmd:
            ...
            description = ("Option '%s' in section '%s'" %
                           (self._get_name(gcmd), self._config.section))
    ...
    def _get_int(self, gcmd, default, minval, maxval):
        get = gcmd.get_int if gcmd else self._config.getint
        return get(self._get_name(gcmd), default or self.default,
            minval or self.minval, maxval or self.maxval)

    def _get_float(self, gcmd, default, minval, maxval,
                  above, below):
        get = gcmd.get_float if gcmd else self._config.getfloat
        return get(self._get_name(gcmd), default or self.default,
            minval or self.minval, maxval or self.maxval,
            above or self.above, below or self.below)

    def _get_float_list(self, gcmd, default, above, below):
        # this code defaults to the empty list, never return None
        default = default or (self.default or [])
        if gcmd:
            ...
            float_list = self._config.getfloatlist(
                            self._get_name(gcmd), default=default)
        if float_list:
            self._validate_float_list(gcmd, float_list, above, below)
        return float_list

    def get(self, gcmd=None, default=None, minval=None, maxval=None,
                     above=None, below=None):
        if self._type_name == 'int':
            return self._get_int(gcmd, default, minval, maxval)
        elif self._type_name == 'float':
            return self._get_float(gcmd, default, minval, maxval, above, below)
        else:
            return self._get_float_list(gcmd, default, above, below)
...
class ContinuousTareFilterHelper:
    def __init__(self, config, sensor):
        ...
        self._sos_filter = self._create_filter(
            self._active_design.design_filter(self._config.error))
    ...
    def update_from_command(self, gcmd, cq=None):
        ...
        self._sos_filter.change_filter(
            self._active_design.design_filter(gcmd.error), cq)
...
class LoadCellProbeConfigHelper:
    def __init__(self, config, load_cell_inst):
        ...
        self._tare_time_param = floatParamHelper(self._config, 'tare_time',
            default=4. / 60., minval=0.01, maxval=1.0)
        # triggering options
        self._trigger_force_param = intParamHelper(self._config,
            'trigger_force', default=75, minval=10, maxval=250)
        self._force_safety_limit_param = intParamHelper(self._config,
            'force_safety_limit', minval=100, maxval=5000, default=2000)
        # pullback move
        self._pullback_distance_param = floatParamHelper(self._config,
        ...
        self._pullback_speed_param = floatParamHelper(self._config,
            'pullback_speed', minval=0.1, maxval=1.0, default=sps * 0.001)
        self._bad_tap_retries_param = intParamHelper(self._config,
...
class LoadCellProbingMove:
    ...
    def _pause_and_tare(self, gcmd):
        ...
        self._continuous_tare_filter_helper.update_from_command(gcmd,
                                    self._mcu_load_cell_probe.get_cmd_queue())
...
class LoadCellPrinterProbe:
    ...
    def _lookup_object(self, key, default):
        config_section_name = self._config.get(key, default=None)
