...
gcode:
    ...
    # perform a single load cell tap which will not fail
    LOAD_CELL_TAP PROBE_SPEED={5 * 60}  # override the speed for faster homing
    ...
    PROBE  # this will allow for failed taps to clean the nozzle and retry
    _HOME_Z_FROM_LAST_PROBE
